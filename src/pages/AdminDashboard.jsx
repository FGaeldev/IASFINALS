...
export default function AdminDashboard() {
  ...
  const handleSaveRole = async (u) => {
    ...
    if (res.success) {
      ...
      setPendingRoles((prev) => { const p = { ...prev }; delete p[u.u_id]; return p; });
    } else {
      alert(res.message);
    }
  };
  ...
  const glass = "bg-white/10 backdrop-blur-lg border border-white/20 text-white shadow-lg";
  const tabStyle = (active) =>
    `px-4 py-2 rounded-full text-sm font-medium transition ${
      active ? "bg-blue-600 text-white" : "bg-white/10 text-gray-200 hover:bg-white/20"
    }`;
  const tableHeader = "sticky top-0 z-10 bg-black/60 backdrop-blur-lg border-b border-white/10";
    ...
    >
      ...
      <div className="relative max-w-6xl mx-auto w-full flex flex-col h-[90vh]">
        ...
        <div className="hidden md:flex gap-3 mb-4">
          <button onClick={() => setTab("users")}    className={tabStyle(tab === "users")}>Users</button>
          <button onClick={() => setTab("logs")}     className={tabStyle(tab === "logs")}>Login Logs</button>
          <button onClick={() => setTab("sessions")} className={tabStyle(tab === "sessions")}>Sessions</button>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {users.map((u) => {
                  ...
                  const dirty = pendingRoles[u.u_id] && pendingRoles[u.u_id] !== u.u_role;
                  return (
                    <tr key={u.u_id} className="border-t border-white/10 hover:bg-white/5">
                      <td className="p-3">{u.u_id}</td>
                      <td className="p-3">{u.u_email}</td>
                      <td className="p-3">
                        <select
                          value={currentRole}
                          onChange={(e) => handleRoleChange(u.u_id, e.target.value)}
              ...
              <tbody>
                {sessions.map((s) => (
                  <tr key={s.s_session_id} className="border-t border-white/10">
                    ...
                    <td className="p-3">{s.s_logout_time ? "Logged out" : "Active"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
